...
## Active Training Runs (launched 2026-02-26 ~03:10 local)
...
```bash
# Quick status — look for curriculum level and escape_rate
tail -30 results/run_V_equal_speed/train.log
tail -30 results/run_W_pursuer95/train.log
tail -30 results/run_X_equal_speed_tier3/train.log
tail -30 results/run_Y_pursuer95_tier3/train.log

# Detailed history (once phases complete)
cat results/run_V_equal_speed/history.json | python -m json.tool

# Check if processes are still running
ps aux | grep train_amsdrl | grep python | grep -v grep
```
