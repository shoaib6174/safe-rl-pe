"""Diagnostic: train pursuer against fixed GreedyEvaderPolicy using SAC.

SAC (Soft Actor-Critic) is off-policy with a replay buffer, making it
better suited for sparse reward problems like pursuit under combined
masking where captures are rare events.
"""
import argparse
import json
import time
...
def main():
    parser = argparse.ArgumentParser(
        description="Train pursuer (SAC) against fixed greedy evader")
    parser.add_argument("--output", type=str,
                        default="results/pursuer_vs_greedy_sac",
    ...
    print("=" * 60)
    print("Diagnostic: Pursuer (SAC) vs Greedy Evader")
    ...
    # SAC uses a single env (no vectorized needed, but we can use 1)
    env = make_env(args.seed, greedy_evader, env_kwargs,
    ...
    print(f"\nTraining pursuer with SAC ({args.total_steps:,} steps)...")
