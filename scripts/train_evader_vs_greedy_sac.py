"""Diagnostic: train evader against fixed GreedyPursuerPolicy using SAC.

SAC (Soft Actor-Critic) is off-policy with a replay buffer, making it
better suited for sparse reward problems like evasion under combined
masking where timeouts (escapes) are rare events early in training.
"""
import argparse
import json
import time
...
def main():
    parser = argparse.ArgumentParser(
        description="Train evader (SAC) against fixed greedy pursuer")
    parser.add_argument("--output", type=str,
                        default="results/evader_vs_greedy_sac",
    ...
    parser.add_argument("--greedy_full_obs", action="store_true",
                        help="Give greedy pursuer unmasked observations.")
    args = parser.parse_args()

    # Parse ent_coef: "auto" or float
    ...
    print("=" * 60)
    print("Diagnostic: Evader (SAC) vs Greedy Pursuer")
    print(f"  Algorithm: SAC (off-policy, replay buffer)")
    print(f"  Arena: {args.arena_width}x{args.arena_height}")
    print(f"  Pursuer speed: {args.pursuer_v_max}x (greedy)")
    ...
    if args.partial_obs:
        ...
        greedy_obs_mode = ("FULL (unmasked)" if args.greedy_full_obs
                           else "MASKED (same as evader)")
        print(f"  Greedy pursuer obs: {greedy_obs_mode}")
    ...
    # SAC uses a single env (off-policy, no need for vectorized)
    env = make_env(args.seed, greedy_pursuer, env_kwargs,
    ...
    print(f"\nTraining evader with SAC ({args.total_steps:,} steps)...")
    ...
    print(f"Best escape rate:  {best_escape_rate:.3f} "
          f"(at step {best_escape_step:,})")
