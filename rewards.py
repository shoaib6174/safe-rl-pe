...
def in_field_of_view(
    ...
    """Check if target is within observer's FOV cone.

    The FOV is a triangle (cone in 2D) centered on the observer's heading
    direction, with half-angle = fov_angle/2 and max range = sensing_radius.

    Args:
        observer_state: Observer state [x, y, theta].
        target_pos: Target position [x, y] or [x, y, theta].
        fov_angle: Total FOV angle in radians (e.g. 2*pi/3 for 120 degrees).
        sensing_radius: Maximum sensing distance.

    Returns:
        True if target is within the FOV cone.
    """
    dx = target_pos[0] - observer_state[0]
    dy = target_pos[1] - observer_state[1]
    ...
    if dist < 1e-12:
        return True  # Same position, always visible

    # Bearing from observer to target (world frame)
    bearing = np.arctan2(dy, dx)

    # Relative angle: difference between bearing and observer heading
    rel_angle = bearing - observer_state[2]
