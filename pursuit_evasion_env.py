...
from envs.rewards import RewardComputer, in_field_of_view, line_of_sight_blocked, nearest_obstacle_distance
...
class PursuitEvasionEnv(gym.Env):
    ...
    def __init__(
        ...
        # Phase 3: FOV triangle (directional sensing cone)
        self.fov_angle = fov_angle  # radians, None = omnidirectional
    ...
    def step(
        ...
        if self.search_tracker is not None:
            evader_visible = self._is_visible(self.pursuer_state, self.evader_state)
    ...
    def _is_visible(self, observer_state: np.ndarray, target_state: np.ndarray) -> bool:
        """Check if target is visible from observer under current partial obs settings.

        Combines FOV cone, sensing radius, and LOS obstacle checks.
        Returns True if target is visible (not masked).
        """
        if not self.partial_obs:
            return True

        if self.fov_angle is not None:
            # FOV cone check (includes range check via sensing_radius)
            radius = self.sensing_radius if self.sensing_radius is not None else 1e6
            if not in_field_of_view(observer_state, target_state, self.fov_angle, radius):
                return False
            # If combined masking, also check LOS through obstacles
            if self.combined_masking:
                return not line_of_sight_blocked(observer_state, target_state, self.obstacles)
            return True

        if self.sensing_radius is not None:
            dx = observer_state[0] - target_state[0]
            dy = observer_state[1] - target_state[1]
        ...
        # LOS-only: mask if obstacle blocks line of sight
        return not line_of_sight_blocked(observer_state, target_state, self.obstacles)

    def _get_obs(self) -> dict:
        """Build observations for both agents."""
        masked = not self._is_visible(self.pursuer_state, self.evader_state)

        # Asymmetric obs: only pursuer is masked, evader always has full info
        if self.asymmetric_obs:
            evader_masked = False
        else:
            evader_masked = not self._is_visible(self.evader_state, self.pursuer_state)
