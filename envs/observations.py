...
Phase 3: Partial observability — LOS masking (15 + 2*K per agent).

Per-agent observation (Phase 2 — full obs, with obstacles):
    obs = [
...
Per-agent observation (Phase 3 — partial obs, with obstacles):
    obs = [
        x_self, y_self, theta_self,         # own state (always visible)
        v_self, omega_self,                  # own velocity (always visible)
        x_opp, y_opp, theta_opp,            # 0.0 if LOS blocked
        v_opp, omega_opp,                   # 0.0 if LOS blocked
        distance_to_opponent,               # 0.0 if LOS blocked
        bearing_to_opponent,                # 0.0 if LOS blocked
        d_to_nearest_wall_x,               # always visible
        d_to_nearest_wall_y,               # always visible
        los_visible,                        # 1.0 if opponent visible, 0.0 if blocked
        obs1_dist, obs1_bearing,           # nearest obstacle (if n_obstacle_obs > 0)
        obs2_dist, obs2_bearing,           # second nearest
        ...                                # up to K obstacles
    ]
...
class ObservationBuilder:
    ...
    Args:
        ...
        partial_obs: Enable Phase 3 partial observability (LOS masking).
    """
    ...
    def __init__(
        ...
        # Phase 3 partial obs adds 1 dim for los_visible flag
        self.obs_dim = 14 + (1 if partial_obs else 0) + 2 * n_obstacle_obs
    ...
    def build(
        ...
        Args:
            ...
            los_blocked: If True and partial_obs enabled, mask opponent features.
        ...
        # Should we mask opponent info this step?
        mask_opp = self.partial_obs and los_blocked
        ...
        theta_o_norm = 0.0 if mask_opp else theta_o / np.pi

        # Normalize velocities
        v_s_norm = v_s / self.v_max
        omega_s_norm = omega_s / self.omega_max
        v_o_norm = 0.0 if mask_opp else v_o / self.v_max
        omega_o_norm = 0.0 if mask_opp else omega_o / self.omega_max

        # Distance to opponent (normalized by arena diagonal)
        dx = x_o - x_s
        dy = y_o - y_s
        distance = np.sqrt(dx**2 + dy**2)
        distance_norm = 0.0 if mask_opp else distance / self.arena_diagonal
        ...
        bearing_norm = 0.0 if mask_opp else bearing_rel / np.pi
        ...
        # Phase 3: add LOS visibility flag
        if self.partial_obs:
            base_obs.append(0.0 if los_blocked else 1.0)
