...
# SP15e_s42: EWC + gamma=0.9, fixed thr (OOM'd)
...
# SP15g_s43: w_vis_pursuer=0.1 + gamma=0.9 (OOM'd)
nohup .venv/bin/python scripts/train_amsdrl.py $BASE \
  --gamma 0.9 --capture_bonus 20 --timeout_penalty -20 \
  --freeze_switch_threshold 0.8 \
  --w_vis_pursuer 0.1 \
...
# SP15i: gamma=0.9, no obstacles (both OOM'd)
...
# SP15j_s42: EWC + schedule + pure SP (OOM'd)
nohup .venv/bin/python scripts/train_amsdrl.py $BASE \
  --gamma 0.9 --capture_bonus 20 --timeout_penalty -20 \
...
# SP15k: w_vis_pursuer + schedule + pure SP (both OOM'd)
for SEED in 42 43; do
  nohup .venv/bin/python scripts/train_amsdrl.py $BASE \
    --gamma 0.9 --capture_bonus 20 --timeout_penalty -20 \
    --freeze_switch_threshold 0.8 $SCHED \
